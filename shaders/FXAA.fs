...
uniform float u_lumaThreshold = 0.125;
uniform float u_mulReduce = 1. / 8.;
uniform float u_minReduce = 1. / 128.;
uniform float u_maxSpan = 8.;
...
void main(void) {
  vec2 texelStep = 1. / textureSize(u_colorTexture, 0);
  ...
  // Sampling neighbour texels. Offsets are adapted to OpenGL texture
  // coordinates.
  ...
  // texels, the smaller the final sampling step direction. This results, that
  // brighter areas are less blurred/more sharper than dark areas.
  ...
  vec3 rgbSampleNeg =
      texture(u_colorTexture, TexCoords + samplingDirection * (1.0 / 3.0 - 0.5))
          .rgb;
  vec3 rgbSamplePos =
      texture(u_colorTexture, TexCoords + samplingDirection * (2.0 / 3.0 - 0.5))
          .rgb;
  ...
  vec3 rgbSampleNegOuter =
      texture(u_colorTexture, TexCoords + samplingDirection * (0.0 / 3.0 - 0.5))
          .rgb;
  vec3 rgbSamplePosOuter =
      texture(u_colorTexture, TexCoords + samplingDirection * (3.0 / 3.0 - 0.5))
          .rgb;
